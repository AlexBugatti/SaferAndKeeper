...
final class InputPinViewController: UIViewController {
    ...
    private func setup() {
        ...
        for index in 0...5 {
            ...
            dotView.layer.borderColor = UIColor(hexString: "#78788C").cgColor
    ...
    private func authInput() {
        clearDots()
        self.titleLabel.text = "Enter master code"
    }
    
    private func firstInput() {
        clearDots()
        self.titleLabel.text = "Enter new master code"
    }
    
    private func retryInput() {
        clearDots()
        self.titleLabel.text = "Retry new master code"
    }
    
    private func clearDots() {
        numbers = []
        dotsStackView.arrangedSubviews.forEach { (dView) in
            dView.layer.borderColor = UIColor(hexString: "#78788C").cgColor
    ...
    private func updatePins() {
        for (_, view) in self.dotsStackView.arrangedSubviews.enumerated() {
            if view.tag < numbers.count {
                view.backgroundColor = UIColor(hexString: "#78788C")
            } else {
